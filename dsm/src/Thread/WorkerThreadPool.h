...
namespace dsm {
// This thread pool works for numberOfThreads > 1. If its value is 1 it will
// execute everything in the main thread. If you want to use a unique parallel
// thread use WorkerThread instead. If (blockUntilFinish == false) the thread
// finishes after the end of the job currently running. Execution of other
// pending jobs is not guaranteed after wantExit is set.
class WorkerThreadPool {
   public:
    inline WorkerThreadPool(size_t numberOfThreads)
        : wantExit(false),
          allDone(true),
          numThreads(numberOfThreads > 0
                         ? numberOfThreads
        ...
        if (this->numThreads != 1) {
            ...
            for (size_t i = 0; i < this->numThreads; ++i) {
                ...
                this->workers.emplace_back(&WorkerThreadPool::workerLoop, this,
                                           i);
            }
        }
    }

    inline ~WorkerThreadPool() { this->join(false); }

    inline size_t getNumThreads() { return this->numThreads; }
        ...
        while (true) {
            // pick the first job in the queue or wait otherwise
            {
                ...
                if (this->taskQueue.empty()) {
                    ...
                    if (allFinished) {
                        {
                            std::lock_guard<std::mutex> waitLock(
                                this->waitMutex);
                            this->allDone = true;
                        }
                    ...
                    // wait for next task
                    this->queueSignal.wait(lock, [&]() {
                        return this->wantExit || !this->taskQueue.empty();
                    });
                }
