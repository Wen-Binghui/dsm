...
inline Buffer<Type>* BufferPool<Type>::popBuffer(const size_t numElements) {
    ...
    if (this->availableBuffers.find(numElements) !=
        this->availableBuffers.end()) {
        std::deque<Buffer<Type>*>& bufferVector =
            this->availableBuffers[numElements];
    ...
    std::unique_ptr<Buffer<Type>> newBuffer =
        std::make_unique<Buffer<Type>>(numElements, id);
...
inline void BufferPool<Type>::pushBuffer(Buffer<Type>* buffer) {
    ...
    if (this->availableBuffers.find(buffer->numElements) ==
        this->availableBuffers.end()) {
        this->availableBuffers.emplace(buffer->numElements,
                                       std::deque<Buffer<Type>*>());
    }
