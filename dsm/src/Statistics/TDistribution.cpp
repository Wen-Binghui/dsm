...
bool TDistribution::empty() const {
    return (this->nu_ <= 0.f || this->sigma_ <= 0.f);
}
...
float TDistribution::nu() const { return this->nu_; }

float TDistribution::mu() const { return this->mu_; }

float TDistribution::sigma() const { return this->sigma_; }
...
float TDistribution::pdf(float x) const {
    ...
    const float a =
        exp(lgamma(0.5f * (this->nu_ + 1.f)) - lgamma(0.5f * this->nu_));
...
float TDistribution::cdf(float x, Tail tail) const {
    ...
    float cdf = 0.5f * betainc(s, 0.5f * this->nu_, 0.5f,
                               lower);  // Eq. 26.7.1 & 26.5.27

    // tail selection
    if ((t > 0 && tail == lower) || (t < 0 && tail == upper)) {
        cdf = 1.f - cdf;  // 26.5.2
    }
...
float TDistribution::icdf(float p, Tail tail) const {
    ...
    const float s =
        betaincinv(2.f * p, 0.5f * this->nu_, 0.5f, lower);  // s >= 0

    // Eq. 26.7.1 & 26.5.27
    const float onems = 1.f - s;
    float t = sqrt(this->nu_ * onems / s);  // t >= 0

    if ((!tailChanged && tail == lower) || (tailChanged && tail == upper)) {
        // t <= 0
        t = -t;
    }
...
float TDistribution::weight(float energy) const {
    ...
    // weight calculation
    const float weight =
        (this->nu_ + 1.f) / (this->nu_ + energyNorm * energyNorm);
...
__m128 TDistribution::weightSSE(__m128 energy) const {
    ...
    // normalize with scale and location
    const __m128 energyNorm = _mm_mul_ps(_mm_sub_ps(energy, this->mu_sse),
                                         _mm_rcp_ps(this->sigma_sse));

    // weight calculation
    const __m128 den =
        _mm_add_ps(this->nu_sse, _mm_mul_ps(energyNorm, energyNorm));
...
std::shared_ptr<TDistribution> TDistribution::fitdist(
    const std::vector<float>& dist, const Eigen::VecXf& fixed) {
    if (!(fixed.size() == 0 || fixed.size() == 3)) {
        std::cout
            << "TDistribution::fitdist() requires a valid fixed vector \n";
    ...
    if (!fixNu && !fixMu && !fixSigma) {
        ...
        // fix mu
        const Eigen::Vec2f params =
            nm.minimize(NLLmu(fixed[1]), start, dist, options);
        return std::make_shared<TDistribution>(params[0], fixed[1], params[1]);
    } else if (fixNu && fixMu && !fixSigma) {
        // fix nu & mu
        const Eigen::VecXf params =
            nm.minimize(NLLmunu(fixed[0], fixed[1]), start, dist, options);
        return std::make_shared<TDistribution>(fixed[0], fixed[1], params[0]);
    } else {
        std::cout << "TDistribution::fitdist() requires a valid parameter "
                     "composition \n";
        return std::make_shared<TDistribution>(0.f, 0.f, 0.f);
    }
}

Eigen::VecXf TDistribution::initialize(const std::vector<float>& dist,
...
float TDistribution::negLogLikelihood(float nu, float mu, float sigma,
    ...
    const float c = lgamma(k) - lgamma(0.5f * nu) + 0.5f * nu * log(nu) -
                    log(sigma) - 0.5f * log(Settings::PI);
    ...
    for (int i = 0; i < numSSE; i += 4) {
        const __m128 t =
            _mm_mul_ps(_mm_sub_ps(_mm_load_ps(dataPtr + i), mmu), msigma);
...
float TDistribution::NLL::operator()(const Eigen::Vec3f& params,
    ...
    // calculate the negative log likelihood
    return TDistribution::negLogLikelihood(params[0], params[1], params[2],
                                           data);
}

TDistribution::NLLmu::NLLmu(float mu) : mu_(mu) {}
...
float TDistribution::NLLmu::operator()(const Eigen::Vec2f& params,
    ...
    // calculate the negative log likelihood
    return TDistribution::negLogLikelihood(params[0], this->mu_, params[1],
                                           data);
}

TDistribution::NLLmunu::NLLmunu(float nu, float mu) : nu_(nu), mu_(mu) {}

float TDistribution::NLLmunu::operator()(const Eigen::VecXf& params,
                                         const std::vector<float>& data) const {
    ...
    // calculate the negative log likelihood
    return TDistribution::negLogLikelihood(this->nu_, this->mu_, params[0],
                                           data);
}
}  // namespace dsm