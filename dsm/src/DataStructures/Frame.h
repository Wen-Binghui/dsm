...
class DSM_EXPORTS_DLL Frame {
   public:
    ...
    const float* gradient(int32_t level)
        const;  // squared norm of gradient magnitude grad = gx*gx + gy*gy
    ...
    void setTrackingResult(Frame* const parent,
   ...
   private:
    ...
    // candidate points
    std::vector<std::unique_ptr<CandidatePoint>>
        candidates_;  // candidates to become active points

    // active points
    std::vector<std::unique_ptr<ActivePoint>>
        activePoints_;  // active points in current window
