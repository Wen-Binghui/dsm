...
class AffineLight {
   public:
    ...
    // Input: exp(-alpha_from)*(I_from - beta_from) = exp(-alpha_to)*(I_to -
    // beta_to) Output: I_from = exp(-alpha)*(I_to - beta)
    static AffineLight calcRelative(const AffineLight& fromGlobal,
                                    const AffineLight& toGlobal);

    // Calculates the global affine light of a camera
    // Input: global of "from" and relative to "from" of "to"
    // Output: global of "to"
    static AffineLight calcGlobal(const AffineLight& fromGlobal,
