...
void CovisibilityGraph::clear() { this->nodes.clear(); }

void CovisibilityGraph::addNode(const std::shared_ptr<Frame>& frame) {
    if (frame->graphNode == nullptr) {
        // create node
        std::unique_ptr<CovisibilityNode> node =
            std::make_unique<CovisibilityNode>(frame);
        frame->graphNode = node.get();  //: frame 放 node, node 里放 &frame
...
void CovisibilityGraph::connect(const std::shared_ptr<Frame>& frame1,
...
void CovisibilityGraph::disconnect(const std::shared_ptr<Frame>& frame1,
                                   const std::shared_ptr<Frame>& frame2) {
    assert(frame1->graphNode);
    assert(frame2->graphNode);
...
void CovisibilityGraph::getAllConnections(const std::shared_ptr<Frame>& frame,
    ...
    for (auto it = frame->graphNode->edges.begin();
         it != frame->graphNode->edges.end(); ++it) {
        connections.push_back(it->first->node);
    }
}

void CovisibilityGraph::getConnectionsByWeight(
    const std::shared_ptr<Frame>& frame, int weight,
    std::vector<Frame*>& connections) {
    assert(frame->graphNode);
