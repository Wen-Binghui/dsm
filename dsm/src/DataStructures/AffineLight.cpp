...
float AffineLight::alpha() const { return this->alpha_; }

float AffineLight::beta() const { return this->beta_; }

Eigen::Vector2f AffineLight::toEigenVec() const {
    return Eigen::Vector2f(this->alpha_, this->beta_);
}

float AffineLight::a() const { return exp(-alpha_); }

float AffineLight::b() const { return -exp(-alpha_) * this->beta_; }
...
AffineLight AffineLight::calcRelative(const AffineLight& fromGlobal,
                                      const AffineLight& toGlobal) {
...
AffineLight AffineLight::calcGlobal(const AffineLight& fromGlobal,
                                    const AffineLight& toRelative) {
    float toGlobalAlpha = toRelative.alpha() + fromGlobal.alpha();
    float toGlobalBeta =
        toRelative.beta() + fromGlobal.beta() * exp(toRelative.alpha());
