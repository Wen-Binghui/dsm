...
inline void ImagePyramid<T>::initialize(int lvl) {
    ...
    // double check to allow multiple threads call this function but compute
    // only once
    ...
    // allocate memory
    this->imagePyramid[lvl] =
        BufferPool<T>::getInstance().popBuffer(destWidth * destHeight);
    ...
    if (wminusgap % 8 == 0) {
        ...
        for (int y = 0; y < hminusgap; y += 2) {
            ...
            for (int x = 0; x < wminusgap; x += 8) {
                ...
                const __m128 sum =
                    _mm_hadd_ps(_mm_add_ps(_mm_load_ps(rowSource),
                                           _mm_load_ps(rowSource + width)),
                                _mm_add_ps(_mm_load_ps(rowSource + 4),
                                           _mm_load_ps(rowSource + 4 + width)));
