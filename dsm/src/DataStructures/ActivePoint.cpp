...
namespace dsm {
ActivePoint::ActivePoint(int32_t creationID,
                         const std::unique_ptr<CandidatePoint>& candidate)
    ...
    for (int lvl = 0; lvl < numPyramids; ++lvl) {
        ...
        for (int idx = 0; idx < Pattern::size(); ++idx) {
            ...
            if (!(uj > 1.1f && uj < (width - 2.1f) && vj > 1.1f &&
                  vj < (height - 2.1f))) {
            ...
            // color
            this->color_[lvl][idx] =
                bilinearInterpolation(image, uj, vj, width);

            // weight = sqrt( c^2 / (c^2 + gradj^2) )
            const float gradj = bilinearInterpolation(grad, uj, vj, width);
            this->weights_[lvl][idx] =
                sqrtf(weightConstant / (weightConstant + gradj));
        }
    }
...
int32_t ActivePoint::currentID() const { return this->currentID_; }

float ActivePoint::u(int32_t lvl) const { return this->u_[lvl]; }

float ActivePoint::v(int32_t lvl) const { return this->v_[lvl]; }

Frame* ActivePoint::reference() const { return this->refFrame_; }
...
float ActivePoint::iDepth() const { return this->iDepth_; }

void ActivePoint::setIDepth(float value) { this->iDepth_ = value; }
...
const std::unordered_map<Frame*, std::unique_ptr<PhotometricResidual>>&
ActivePoint::observations() const {
    return this->observations_;
}

void ActivePoint::addObservation(Frame* const kf,
...
float ActivePoint::iDepthHessian() const { return this->iDepthHessian_; }

void ActivePoint::setIDepthHessian(float H) { this->iDepthHessian_ = H; }

float ActivePoint::parallax() const { return this->parallax_; }

void ActivePoint::setParallax(float p) { this->parallax_ = p; }
}  // namespace dsm