...
Frame::Frame(int id, double timestamp, unsigned char* image)
    ...
    if (settings.useTDistribution) {
        errorDist = std::make_shared<TDistribution>(
            settings.defaultNu, settings.defaultMu, settings.defaultSigma);
    } else {
        errorDist = std::make_shared<RobustNormalDistribution>(
            settings.defaultMu, settings.defaultSigma);
    }
...
void Frame::activate() { this->status_ = Status::ACTIVE; }
...
bool Frame::isActive() { return (this->status_ == Status::ACTIVE); }

int Frame::frameID() const { return this->frameID_; }

int Frame::keyframeID() const { return this->keyframeID_; }

void Frame::setKeyframeID(int id) { this->keyframeID_ = id; }

int Frame::activeID() const { return this->activeID_; }

void Frame::setActiveID(int id) { this->activeID_ = id; }

double Frame::timestamp() const { return this->timestamp_; }

Frame::Type Frame::type() const { return this->type_; }
...
void Frame::evolveToKeyframe() {
    ...
    if (this->trackingParent_) {
        this->camToWorld_ =
            this->trackingParent_->camToWorld() * this->thisToParentPose_;

        this->affineLight_ = AffineLight::calcGlobal(
            this->trackingParent_->affineLight(), this->thisToParentLight_);
    } else {
        // first keyframe
    ...
    this->frameBlock_ = std::make_unique<FrameParameterBlock>(
        this->camToWorld_.cast<double>(), this->affineLight_);
}
...
Frame* const Frame::parent() const { return this->trackingParent_; }
...
void Frame::setTrackingResult(Frame* const parent,
                              const Sophus::SE3f& thisToParentPose,
...
void Frame::setFlaggedToDrop(bool flag) { this->flaggedToDrop_ = flag; }

bool Frame::flaggedToDrop() const { return this->flaggedToDrop_; }

void Frame::setErrorDistribution(const std::shared_ptr<IDistribution>& dist) {
    const auto& settings = Settings::getInstance();
    this->errorDistribution_ = dist;
    this->energyThreshold_ =
        this->errorDistribution_->icdf(settings.inlierPercentile);
}
