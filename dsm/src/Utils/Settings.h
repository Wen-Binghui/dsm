...
class Settings {
   ...
   public:
    ...
    float maxPixelOutlier;   // maximum number of pixels to consider the point
                             // outlier
    float maxPixelDiscard;   // maximum number of pixels to discard from
                             // optimization
    ...
    // candidate selection
    int pointDetectionLevels;   // maximum number of pyramid levels to detect
                                // candidate points
    int numCandidates;          // maximum number of candidates per frame
    int numBlocksPerDimension;  // number of block in "x" and "y" image
                                // dimensions
    float minGradAdd;  // threshold constant addition to histogram for candidate
                       // pixel detection
    ...
    // candidate tracking
    float maxEplLengthFactor;  // maximum length factor of epl to search based
                               // on width and height
    ...
    int subpixelIterations;    // number of gauss newton iterations for subpixel
                               // refinement
    float subpixelStepThreshold;  // if step < thr the gauss newton optimization
                                  // stops
    ...
    int candidateOptIterations;     // number of iterations for candidate
                                    // refinement
    int minDistToActivate;          // minimum neighbour distance to activate
    float maxCandidateUncertainty;  // maximum match uncertainty
    float minCandidateQuality;  // minimum distance to the second best match to
                                // activate
    ...
    bool printSummary;
    bool showFullReport;
    float minOptimizationGrad;  // minimum gradient to consider a observation as
                                // outlier
    ...
    int maxTemporalKeyframes;  // maximum number of temporal active keyframes in
                               // the opt window
    int maxCovisibleKeyframes;   // maximum number of covisible active keyframes
                                 // in the opt window
    int numAlwaysKeepKeyframes;  // number of temporal keyframes to always keep
                                 // in the optimization window + the new one

    float minPointCovisible;  // minimum ratio of visible points to consider a
                              // keyframe as covisible
    float maxLightCovisible;  // maximum light change to consider a keyframe
                              // covisible

    // Outlier removal
    int minNumKFToConsiderNew;   // minimum number of keyframes to consider an
                                 // active point as a new (temporal)
    int minNumGoodObservations;  // minimum number of observation to consider an
                                 // active point as inlier

    // Keyframe Selection criteria
    float newKFDistWeight;      // weight of the camera translation relative to
    ...
    int minNumMappedFramesToCreateKF;  // minimum number of tracked frames to
                                       // create a new one
   ...
   private:
    ...
    bool checkParameter(const std::string& name, const std::string& nameValue,
