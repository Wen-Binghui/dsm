...
FullSystem::FullSystem(int w, int h, const Eigen::Matrix3f& calib,
    ...
    // thread pool
    this->threadPool =
        std::make_shared<WorkerThreadPool>(settings.mappingThreads);

    // initializer
    this->initializer = std::make_unique<MonoInitializer>(this->threadPool,
                                                          this->outputWrapper);

    // pixel detector
    this->pointDetector = std::make_unique<PointDetector>(
        width, height, (int)settings.pointDetectionLevels,
        (int)settings.numBlocksPerDimension, settings.minGradAdd,
        this->threadPool);

    this->pixelMask = (int32_t*)Eigen::internal::aligned_malloc(
    ...
    this->trackingReference =
        std::make_shared<FrameTrackerReference>(width, height);
    this->newTrackingReference =
        std::make_shared<FrameTrackerReference>(width, height);
    ...
    this->ceresOptimizer =
        std::make_unique<CeresPhotometricBA>(config, this->outputWrapper);
    ...
    this->depthMapImage = cv::Mat(h >> settings.showDepthMapLvl,
                                  w >> settings.showDepthMapLvl, CV_8UC3);
    ...
    if (!settings.singleThreaded) {
        this->mappingThread =
            std::make_unique<std::thread>(&FullSystem::mappingThreadLoop, this);
    }
}
...
bool FullSystem::isInitialized() const { return this->initialized; }

bool FullSystem::isLost() const { return !this->trackingIsGood; }

void FullSystem::getTrajectory(std::vector<Eigen::Matrix4f>& poses,
    ...
    for (const auto& kf : allKeyframes) {
        ...
        for (const auto& pt : kf->activePoints()) {
            ...
            for (int idx = 0; idx < Pattern::size(); ++idx) {
                ...
                // point in camera
                Eigen::Vector3f pt =
                    depth * Eigen::Vector3f((uj - K(0, 2)) / K(0, 0),
                                            (vj - K(1, 2)) / K(1, 1), 1.f);
...
bool FullSystem::getLastWasKF() const { return this->lastWasKF; }
...
bool FullSystem::initialize(const std::shared_ptr<Frame>& frame) {
    ...
    if (allKeyframes.empty()) {
        ...
        if (this->initializer->initialize(frame, firstToSecond)) {
            ...
            frame->setTrackingResult(firstKF.get(), firstToSecond.inverse(),
                                     AffineLight());
            ...
            std::cout << "Done initialization in " << Utils::elapsedTime(t1, t2)
                      << "ms" << std::endl;
    ...
    Utils::Time t2 = std::chrono::steady_clock::now();
    std::cout << "Trying to initialize... " << Utils::elapsedTime(t1, t2)
              << "ms" << std::endl;
...
void FullSystem::trackFrame(int id, double timestamp, unsigned char* image) {
    ...
    std::shared_ptr<Frame> trackingNewFrame =
        std::make_shared<Frame>(id, timestamp, image);
    ...
    if (settings.debugPrintLog && settings.debugLogTracking) {
        ...
        std::string msg =
            "trackingPose: " + std::to_string(pose[0]) + "\t" +
            std::to_string(pose[1]) + "\t" + std::to_string(pose[2]) + "\t" +
            std::to_string(pose[3]) + "\t" + std::to_string(pose[4]) + "\t" +
            std::to_string(pose[5]) + "\t" + std::to_string(pose[6]) + "\t";
        msg += "affineLight: " + std::to_string(light.alpha()) + "\t" +
               std::to_string(light.beta()) + "\t";
    ...
    if (!this->createNewKeyframe) {
        if (this->numMappedFramesFromLastKF >=
            settings.minNumMappedFramesToCreateKF) {
            localCreateNewKeyframe =
                this->isNewKeyframeRequired(trackingNewFrame);
        }
    ...
    if (!settings.singleThreaded) {
        {
            std::lock_guard<std::mutex> unMappedLock(
                this->unmappedTrackedFramesMutex);
            ...
            {
                std::lock_guard<std::mutex> newFrameMappedLock(
                    this->newFrameMappedMutex);
    ...
    if (this->outputWrapper) {
        ...
        const Eigen::Matrix4f camPose =
            (this->lastTrackedFrame->parent()->camToWorld() *
    ...
    if (!settings.singleThreaded && settings.blockUntilMapped &&
        this->trackingIsGood) {
        this->waitUntilMappingFinished();
    }
}
...
void FullSystem::trackNewFrame(const std::shared_ptr<Frame>& frame) {
    ...
    Sophus::SE3f lastToWorldPose =
        this->lastTrackedFrame->parent()->camToWorld() *
        this->lastTrackedFrame->thisToParentPose();

    AffineLight lastToWorldLight =
        AffineLight::calcGlobal(this->lastTrackedFrame->parent()->affineLight(),
    ...
    if (this->trackingReferenceUpdated) {
        ...
        if (newReference && oldReference) {
            ...
            // pose
            const Sophus::SE3f poseCorrection =
                newReference->camToWorld().inverse() *
                oldReference->camToWorld() * newReference->thisToParentPose();

            lastToWorldPose *= poseCorrection.inverse();

            // light
            const AffineLight newRefOldLight =
                AffineLight::calcGlobal(newReference->parent()->affineLight(),
                                        newReference->thisToParentLight());
            const AffineLight lightCorrection = AffineLight::calcRelative(
                newRefOldLight, newReference->affineLight());
            lastToWorldLight =
                AffineLight::calcGlobal(lastToWorldLight, lightCorrection);
        }
    }
    ...
    const Sophus::SE3f lastToRef =
        reference->camToWorld().inverse() * lastToWorldPose;
    Sophus::SE3f frameToRefPose = lastToRef * this->lastTrackedMotion;

    // affine light prior
    AffineLight frameToRefLight =
        AffineLight::calcRelative(reference->affineLight(), lastToWorldLight);
    ...
    bool goodTracked = this->tracker->trackFrame(
        this->trackingReference, frame, frameToRefPose, frameToRefLight,
    ...
    if (this->trackingReference->firstFrameResidual() < 0.f) {
        this->trackingReference->setFirstFrameResidual(
            this->lastTrackedResidual);
    }
...
bool FullSystem::isNewKeyframeRequired(
    ...
    if ((dist.norm() * settings.newKFDistWeight +
        ...
        settings.newKFResidualWeight *
                this->trackingReference->firstFrameResidual() <
            this->lastTrackedResidual) {
        return true;
    }
...
void FullSystem::mappingThreadLoop() {
    ...
    while (true) {
        ...
        // waiting condition. It will wake up when a new frame is set in the
        // queue or when shutting down
        {
            std::unique_lock<std::mutex> unMappedLock(
                this->unmappedTrackedFramesMutex);
            if (this->unmappedTrackedFrames.empty()) {
                ...
                this->unmappedTrackedFramesSignal.wait(unMappedLock, [&]() {
                    return this->shouldStop ||
                           !this->unmappedTrackedFrames.empty();
                });
            ...
            if (!this->unmappedTrackedFrames.empty()) {
                ...
                // if new keyframe required, discard all the frames until the
                // need was requested
                if (this->createNewKeyframeID >
                    activeKeyframes.back()->frameID()) {
                    while (this->unmappedTrackedFrames.size() > 0 &&
                           this->unmappedTrackedFrames.front()->frameID() <=
                               this->createNewKeyframeID) {
                        frame = std::move(this->unmappedTrackedFrames.front());
                        this->unmappedTrackedFrames.pop_front();
                    }
                }
                ...
                if (this->unmappedTrackedFrames.size() >
                    settings.maxUnmappedFrames) {
...
void FullSystem::waitUntilMappingFinished() {
    ...
    this->newFrameMappedSignal.wait(newFrameMappedLock,
                                    [&]() { return this->newFrameMappedDone; });
}
...
void FullSystem::createCandidates(const std::shared_ptr<Frame>& frame) {
    ...
    int num = this->pointDetector->detect(frame, (int)settings.numCandidates,
                                          this->pixelMask, this->outputWrapper);
    ...
    for (int32_t row = distToBorder; row < height - distToBorder; ++row) {
        for (int32_t col = distToBorder; col < width - distToBorder; ++col) {
            ...
            // create a point
            candidates.emplace_back(std::make_unique<CandidatePoint>(
                (float)col, (float)row, this->pixelMask[idx], frame));
        }
    }
    ...
    if (settings.debugPrintLog && settings.debugLogPixelDetection) {
        const std::string msg =
            "PointDetect: " + std::to_string(candidates.size()) + "\t";
    ...
    // std::cout << "Select pixels: " << Utils::elapsedTime(t1, t2) <<
    // std::endl;
}
...
void FullSystem::trackCandidates(const std::shared_ptr<Frame>& frame) {
    ...
    if (settings.debugCandidates ||
        (settings.debugPrintLog && settings.debugLogCandidatesTracking)) {
        int numGood = 0;
        int numBadError = 0;
        int numBadEpiLine = 0;
        int numOOB = 0;
        ...
        for (const auto& kf : activeKeyframes) {
            ...
            for (const auto& cand : candidates) {
                ...
                if (cand->status() == CandidatePoint::OUTLIER ||
                    ...
                    if (status == CandidatePoint::GOOD)
                        numGood++;
                    else if (status == CandidatePoint::BAD_ERROR)
                        numBadError++;
                    else if (status == CandidatePoint::BAD_EPILINE)
                        numBadEpiLine++;
                    else if (status == CandidatePoint::OOB)
                        numOOB++;
                    else if (status == CandidatePoint::SKIPPED)
                        numSkipped++;
                    else if (status == CandidatePoint::BAD_CONDITIONED)
                        numBadConditioned++;
                    else if (cand->iDepth() < 0.f)
                        numNegativeIDepth++;
                }
            }
        }
        ...
        if (settings.debugCandidates) {
            ...
            std::cout << "Candidates bad epiline: " << numBadEpiLine
                      << std::endl;
            std::cout << "Candidates oob: " << numOOB << std::endl;
            std::cout << "Candidates skipped: " << numSkipped << std::endl;
            std::cout << "Candidates bad_cond: " << numBadConditioned
                      << std::endl;
            std::cout << "Candidates neg_iDepth: " << numNegativeIDepth
                      << std::endl;
            std::cout << std::endl;
        }

        if (settings.debugPrintLog && settings.debugLogCandidatesTracking) {
            const std::string msg = "Candidates: " + std::to_string(numGood) +
                                    "\t" + std::to_string(numBadError) + "\t" +
                                    std::to_string(numBadEpiLine) + "\t" +
                                    std::to_string(numOOB) + "\t" +
                                    std::to_string(numSkipped) + "\t" +
                                    std::to_string(numBadConditioned) + "\t";
        ...
        int start = 0;
        int end = 0;
        int step = (itemsCount + settings.mappingThreads - 1) /
                   settings.mappingThreads;
        ...
        for (int i = 0; i < settings.mappingThreads; ++i) {
            ...
            this->threadPool->addJob(
                PointObserver(toObserve, frame, start, end));
        }
...
void FullSystem::refineCandidates() {
    ...
    for (int j = 0; j < temporalKeyframes.size() - 1; ++j) {
        // relative pose
        const Sophus::SE3f relPose =
            worldToLast * temporalKeyframes[j]->camToWorld();
        ...
        for (auto& cand : candidates) {
            ...
            bool isGoodCandidate =
                cand->matchUncertainty() < settings.maxCandidateUncertainty &&
            ...
            if (!Utils::project(cand->u0(), cand->v0(), cand->iDepth(), width,
                                height, KRKinv, Kt, pointInFrame)) {
                cand = nullptr;
                continue;
            }
    ...
    if (settings.debugCandidates ||
        (settings.debugPrintLog && settings.debugLogCandidatesOpt)) {
        int numGood = 0;
        int numBad = 0;
        int numSkipped = 0;
        ...
        for (int i = 0; i < itemsCount; ++i) {
            ...
            if (toOptimize[i]->status() ==
                CandidatePoint::PointStatus::OPTIMIZED)
                numGood++;
            else if (toOptimize[i]->status() ==
                     CandidatePoint::PointStatus::INITIALIZED)
                numSkipped++;
            else if (toOptimize[i]->status() ==
                     CandidatePoint::PointStatus::OUTLIER)
                numBad++;
        }
        ...
        if (settings.debugPrintLog && settings.debugLogCandidatesOpt) {
            const std::string msg =
                "Candidates Opt: " + std::to_string(numGood) + "\t" +
                std::to_string(numSkipped) + "\t" + std::to_string(numBad) +
                "\t";
        ...
        for (int i = 0; i < settings.mappingThreads; ++i) {
            ...
            this->threadPool->addJob(
                PointOptimizer(toOptimize, activeKeyframes, start, end));
        }
    }
...
void FullSystem::createKeyframeAndOptimize(
    ...
    if (settings.debugPrintLog && settings.debugLogKeyframes) {
        const std::string msg =
            "Keyframe: " + std::to_string(frame->keyframeID()) + "\t";
    ...
    // std::cout << "Refine Candidates: " << Utils::elapsedTime(t_refine_init,
    // t_refine_end) << std::endl;
    ...
    Utils::Time t13 = std::chrono::steady_clock::now();
    {
        ...
        if ((this->outputWrapper && settings.showDepthMap) ||
            ...
            // depth map
            this->drawTrackDepthMap(this->newTrackingReference,
                                    this->depthMapImage,
                                    settings.showDepthMapLvl);
        }
        ...
        if (settings.saveDepthMaps) {
            ...
            std::string file = settings.depthMapsDir + "/depth" +
                               std::to_string(this->saveID) + ".png";
    ...
    // std::cout << "Tracking ref time: " << Utils::elapsedTime(t13, t14) <<
    // std::endl;
    ...
    if (settings.debugShowOptLight && this->outputWrapper) this->drawOptLight();
    ...
    if (this->outputWrapper) {
        ...
        for (const auto& kf : covisibleKeyfames) {
            this->outputWrapper->publishKeyframe(kf,
                                                 KeyframeType::COVISIBILITY);
        }
    }
    Utils::Time t16 = std::chrono::steady_clock::now();
    // std::cout << "Visualization time: " << Utils::elapsedTime(t15, t16) <<
    // std::endl;
    ...
    // std::cout << "Update connectivity time: " << Utils::elapsedTime(t9, t10)
    // << std::endl;
    ...
    // std::cout << "Create candidates time: " << Utils::elapsedTime(t19, t20)
    // << std::endl;

    Utils::Time t2 = std::chrono::steady_clock::now();
    std::cout << "createKeyframeAndOptimize in " << Utils::elapsedTime(t1, t2)
              << std::endl;
}

void FullSystem::drawTrackDepthMap(
    const std::shared_ptr<FrameTrackerReference>& trackRef,
    cv::Mat& depthMapBGR, int lvl) {
    ...
    this->minIDepthTracker =
        std::max(allIDepths[(int)(allIDepths.size() * 0.1f)], 0.f);
    this->maxIDepthTracker =
        std::max(allIDepths[(int)(allIDepths.size() * 0.9f)], 0.f);
    ...
    for (int row = 0; row < h; ++row) {
        for (int col = 0; col < w; ++col) {
            ...
            if (!(iDepthMap[index] <= 0.f)) {
                Eigen::Vector3f rgb =
                    Utils::colorMap(this->minIDepthTracker,
                                    this->maxIDepthTracker, iDepthMap[index]) *
                    255;
                cv::Vec3b bgr =
                    cv::Vec3b((unsigned char)rgb[2], (unsigned char)rgb[1],
                              (unsigned char)rgb[0]);
...
void FullSystem::drawActiveKeyframes() {
    ...
    for (int i = 0; i < numActiveKeyframes - 1; ++i) {
        // relative pose data
        const Sophus::SE3f relPose =
            worldToLast * activeKeyframes[i]->camToWorld();
        const Eigen::Matrix3f KRKinv = K * relPose.rotationMatrix() * Kinv;
        const Eigen::Vector3f Kt = K * relPose.translation();
        ...
        for (const auto& point : activePoints) {
            ...
            if (point->visibility(lastIdx) == Visibility::VISIBLE) {
                Eigen::Vector2f pt2d;
                if (!Utils::project(u, v, point->iDepth(), w, h, KRKinv, Kt,
                                    pt2d)) {
                    continue;
                }

                Eigen::Vector3f rgb =
                    Utils::colorMap(this->minIDepthOpt, this->maxIDepthOpt,
                                    point->iDepth()) *
                    255;

                // draw in the reference image
                cv::rectangle(images[i],
                              cv::Rect((int)u - rectPad, (int)v - rectPad,
                                       rectSize, rectSize),
                              cv::Scalar(rgb[2], rgb[1], rgb[0]), 2);

                // draw in last keyframe
                cv::rectangle(
                    images[numActiveKeyframes - 1],
                    cv::Rect((int)pt2d[0] - rectPad, (int)pt2d[1] - rectPad,
                             rectSize, rectSize),
                    cv::Scalar(rgb[2], rgb[1], rgb[0]), 2);
            } else {
                cv::rectangle(images[i],
    ...
    for (auto& kf : activeKeyframes) {
        ...
        for (auto& point : kf->activePoints()) {
            if (point->visibility(idx) == Visibility::VISIBLE) {
                ...
                Eigen::Vector3f rgb =
                    Utils::colorMap(0.f, 15.f * 15.f * Pattern::size(),
                                    (float)error) *
                    255;
                ...
                cv::circle(
                    errorMap,
                    cv::Point((int)(pt2d[0] + 0.5f), (int)(pt2d[1] + 0.5f)), 2,
                    cv::Scalar(rgb[2], rgb[1], rgb[0]), -1);
            }
        }
    }
    ...
    for (auto& kf : activeKeyframes) {
        ...
        for (auto& point : kf->activePoints()) {
            if (point->visibility(idx) == Visibility::VISIBLE) {
                ...
                Eigen::Vector3f rgb =
                    Utils::colorMap(0.f, 1.f, (float)weight) * 255;
    ...
    for (int i = 0; i < numActiveKeyframes; ++i) {
        ...
        for (const auto& kf2 : activeKeyframes) {
            ...
            for (const auto& point : kf2->activePoints()) {
                if (point->visibility(kf1->keyframeID()) !=
                    Visibility::VISIBLE) {
                    continue;
                }
                ...
                for (int idx = 0; idx < Pattern::size(); ++idx) {
                    ...
                    Eigen::Vector2f pt2d;
                    if (!Utils::project(uj, vj, point->iDepth(), w, h, KRKinv,
                                        Kt, pt2d)) {
                        continue;
                    }

                    float newImgColor =
                        bilinearInterpolation(image, pt2d[0], pt2d[1], w);

                    // image coordiantes
                    int row = static_cast<int>(affLight_a * color[idx] +
                                               affLight_b + 0.5f);
                    ...
                    // draw
                    images[i].at<cv::Vec3b>(254 - row, col) =
                        cv::Vec3b(0, 0, 0);
                }
            }
        }
        ...
        cv::line(images[i], cv::Point(p1.x, 254 - p1.y),
                 cv::Point(p2.x, 254 - p2.y), cv::Scalar(0, 0, 255), 1);
    }
...
void FullSystem::drawOptErrorDist() {
    ...
    for (int i = 0; i < numActiveKeyframes; ++i) {
        ...
        images[i] = Utils::drawDistribution(
            allObservations, distribution, Eigen::Vector2f(-255.f, 255.f),
            Eigen::Vector2f(0, 0.1f), Eigen::Vector2i(510, 510), 510);
    }
...
FullSystem::PointObserver::PointObserver(
    std::vector<CandidatePoint*>& theCandidates,
    const std::shared_ptr<Frame>& targetFrame, int aBegin, int anEnd)
...
FullSystem::PointOptimizer::PointOptimizer(
    std::vector<CandidatePoint*>& theCandidates,
    const std::vector<std::shared_ptr<Frame>>& activeFrames, int aBegin,
    int anEnd)
    : candidates(theCandidates),
