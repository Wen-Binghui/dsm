...
class DSM_EXPORTS_DLL FullSystem {
   public:
    ...
    void getTrajectory(std::vector<Eigen::Matrix4f>& poses,
   ...
   private:
    ...
    Sophus::SE3f lastTrackedMotion;  // last tracked frame motion based on
                                     // constant velocity model
    float lastTrackedResidual;

    // tracking reference
    std::mutex trackingReferenceMutex;
    std::shared_ptr<FrameTrackerReference>
        trackingReference;  // tracking reference
    std::shared_ptr<FrameTrackerReference>
        newTrackingReference;  // updated tracking reference
    ...
    std::atomic_int
        numMappedFramesFromLastKF;  // write in mapping thread, read in tracking

    // Optimization window
    std::unique_ptr<LMCW> lmcw;

    // ceres optimizer
    std::unique_ptr<CeresPhotometricBA>
        ceresOptimizer;  // photometric bundle adjustment
    ...
    struct PointObserver {
        PointObserver(std::vector<CandidatePoint*>& theCandidates,
                      const std::shared_ptr<Frame>& targetFrame, int aBegin,
                      int anEnd);
