...
class PointDetector {
   public:
    ...
    PointDetector(
        int w, int h, int numLevels = 2, int numBlocks = 20,
        float threshold = 7.f,
    ...
    // mask: out selected pixels. if > 0 tells the level in which the pixel was
    // selected return: number of selected pixels
    int detect(const std::shared_ptr<Frame>& frame, int numPoints,
               int32_t* const mask, IVisualizer* outputWrapper = nullptr);
   ...
   private:
    ...
    int selectPixels(const int winSize, int32_t* const mask, int yMin,
                     int yMax) const;

    // function for each coarse window
    int selectMaxPixel(const int row, const int col, const int winSize,
                       int32_t* const mask) const;

    // recursive function for each fine window
    void selectMaxPixelRecursive(
    ...
    int selectBestPixel(const std::vector<CandidatePixel>& salientPixels,
                        const int row, const int col, const int numBlocks,
                        const int lvl, int32_t* const mask) const;

    void selectMaxPixelLvl(const int row, const int col, const int winSize,
                           int& maxLvl, CandidatePixel& candidate) const;
   ...
   private:
    ...
    struct CalcThresholdParallel {
        CalcThresholdParallel(PointDetector& thisDetector, int aBegin,
                              int anEnd, const float* gradientImg);
