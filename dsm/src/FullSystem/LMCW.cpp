...
void LMCW::dropKeyframes() {
    ...
    for (auto it = this->activeKeyframes_.begin();
         it != this->activeKeyframes_.end();) {
        if ((*it)->flaggedToDrop()) {
            (*it)->deactivate();
...
void LMCW::selectTemporalWindow(
    const std::unique_ptr<CeresPhotometricBA>& photometricBA) {
    const auto& settings = Settings::getInstance();
    ...
    for (const std::shared_ptr<Frame>& kf : this->activeKeyframes_) {
        ...
        const AffineLight light = AffineLight::calcRelative(
            kf->affineLight(), lastKeyframe->affineLight());
        ...
        for (const auto& point : kf->activePoints()) {
            ...
            Eigen::Vector2f pt2d;
            if (!Utils::project(point->u(0), point->v(0), point->iDepth(), w, h,
                                KRKinv, Kt, pt2d)) {
            ...
            std::unique_ptr<PhotometricResidual> obs =
                std::make_unique<PhotometricResidual>(point, lastKeyframe,
                                                      photometricBA);
        ...
        if ((kf->keyframeID() <=
             lastKeyframeID - settings.numAlwaysKeepKeyframes) &&
            (this->activeKeyframes_.size() - numFlaggedToDrop) >=
                settings.maxTemporalKeyframes) {
            ...
            if (ratio < settings.minPointCovisible ||
                fabs(light.alpha()) > settings.maxLightCovisible) {
    ...
    if ((this->activeKeyframes_.size() - numFlaggedToDrop) >=
        settings.maxTemporalKeyframes) {
        float maxScore = -std::numeric_limits<float>::max();
        int idx = -1;

        // keep the last N keyframes
        const int maxKeyframeID = (int)this->activeKeyframes_.size() -
                                  settings.numAlwaysKeepKeyframes;
        for (int i = 0; i < maxKeyframeID; ++i) {
            const Sophus::SE3f pose_i =
                this->activeKeyframes_[i]->camToWorld().inverse();
            ...
            for (int j = 0; j < this->activeKeyframes_.size() - 1; ++j) {
                ...
                const Sophus::SE3f relPose =
                    pose_i * this->activeKeyframes_[j]->camToWorld();
                const float dist = relPose.translation().norm() +
    ...
    if (settings.debugPrintLog && settings.debugLogActivePoints) {
        const std::string msg =
            "Act. Temporal: " + std::to_string(this->numActivePoints) + "\t";
    ...
    if (settings.debugPrintLog && settings.debugLogDistanceMap) {
        const std::string msg =
            "DT: " + std::to_string(this->distanceMap_->getNumObstacles()) +
            "\t";
    ...
    if (settings.debugShowDistanceTransformBefore &&
        settings.doOnlyTemporalOpt && this->outputWrapper_) {
...
void LMCW::selectCovisibleWindow(
    const std::unique_ptr<CeresPhotometricBA>& photometricBA) {
    ...
    const std::shared_ptr<Frame> lastActKeyframe =
        this->activeKeyframes_.back();  // make a copy, required!
    ...
    for (const auto& kf : this->allKeyframes_) {
        ...
        for (const auto& point : kf->activePoints()) {
            // project point to new image
            Eigen::Vector2f pt2d;
            if (!Utils::projectAndCheck(point->u(0), point->v(0),
                                        point->iDepth(), K, w, h, R, t, pt2d)) {
    ...
    while (this->temporalWindowIndex < settings.maxCovisibleKeyframes) {
        ...
        const auto maxElement =
            std::max_element(numVisiblePoints.begin(), numVisiblePoints.end());
        const int position =
            (int)std::distance(numVisiblePoints.begin(), maxElement);

        // pick the one with highest number of visible points
        const std::shared_ptr<Frame>& selectedKeyframe =
    ...
    if (this->temporalWindowIndex > 0) {
        ...
        for (int i = 0; i < this->temporalWindowIndex; ++i) {
            for (const auto& point :
                 this->activeKeyframes_[i]->activePoints()) {
                if (point->visibility(lastActID) == Visibility::VISIBLE) {
                    std::unique_ptr<PhotometricResidual> obs =
                        std::make_unique<PhotometricResidual>(
                            point, lastActKeyframe, photometricBA);
...
void LMCW::activatePoints(
    ...
    if (ratio > 1.7f)
        this->minDistToActivate = this->minDistToActivate + 3;
    else if (ratio > 1.4f)
        this->minDistToActivate = this->minDistToActivate + 2;
    else if (ratio > 1.1f)
        this->minDistToActivate = this->minDistToActivate + 1;
    ...
    this->minDistToActivate =
        std::min(this->minDistToActivate, 2 * Pattern::width());

    const float minDist = (float)this->minDistToActivate *
                          this->minDistToActivate;  // squared dist
    ...
    for (int i = this->temporalWindowIndex; i < numActiveKeyframes - 1; ++i) {
        ...
        for (auto& cand : candidates) {
            ...
            if (status == CandidatePoint::OPTIMIZED) {
                ...
                if (this->distanceMap_->dist(x, y) < (minDist * level * level))
                    continue;

                // activePoint from candidate
                std::unique_ptr<ActivePoint> point =
                    std::make_unique<ActivePoint>(lastKeyframe->keyframeID(),
                                                  cand);

                // observations & visibility
                for (const std::shared_ptr<Frame>& frame :
                     this->activeKeyframes_) {
    ...
    if (settings.debugPrintLog && settings.debugLogActivePoints) {
        const std::string msg =
            "Act. New: " + std::to_string(numPointsCreated) + "\t";
...
void LMCW::removeOutliers() const {
    ...
    for (const std::shared_ptr<Frame>& keyframe : this->activeKeyframes_) {
        ...
        for (int i = 0; i < activePoints.size(); ++i) {
            ...
            if (age < settings.minNumKFToConsiderNew &&
                !keyframe->flaggedToDrop()) {
                // for new points check that have been observed in all new
                // keyframes
                if (point->numObservations() < (age + 1)) {
                    remove = true;
                }
            } else {
                // for old points check that have been observed at least in
                // "minNumGoodObservations" keyframes
                if (point->numObservations() <
                    settings.minNumGoodObservations) {
                    remove = true;
                }
            }
...
void LMCW::updateConnectivity() const {
    ...
    for (int i = 0; i < numActiveKeyframes; ++i) {
        for (int j = i + 1; j < numActiveKeyframes; ++j) {
            ...
            this->covisibilityGraph_->connect(this->activeKeyframes_[i],
                                              this->activeKeyframes_[j],
                                              totalVisible);
        }
    }

    if (this->outputWrapper_) {
        this->outputWrapper_->publishCovisibility(
            this->covisibilityGraph_->adjacencyMatrix());
    }
}
}  // namespace dsm