...
class LMCW {
   public:
    ...
    inline const std::vector<std::shared_ptr<Frame>>& activeWindow() const {
        return this->activeKeyframes_;
    }

    // all keyframes
    inline const std::vector<std::shared_ptr<Frame>>& allKeyframes() const {
        return this->allKeyframes_;
    }

    // temporally connected keyframes
    inline std::vector<std::shared_ptr<Frame>> temporalWindow() const {
        return std::vector<std::shared_ptr<Frame>>(
            this->activeKeyframes_.begin() + temporalWindowIndex,
            this->activeKeyframes_.end());
    }

    // covisibility based connected keyframes
    inline std::vector<std::shared_ptr<Frame>> covisibleWindow() const {
        return std::vector<std::shared_ptr<Frame>>(
            this->activeKeyframes_.begin(),
            this->activeKeyframes_.begin() + temporalWindowIndex);
    }

   private:
    // selects the temporal window part
    void selectTemporalWindow(
        const std::unique_ptr<CeresPhotometricBA>& photometricBA);

    // selects the covisible window part
    void selectCovisibleWindow(
        const std::unique_ptr<CeresPhotometricBA>& photometricBA);
