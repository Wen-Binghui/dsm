...
PointDetector::PointDetector(
    int w, int h, int numLevels, int numBlocks, float threshold,
    ...
    this->thresholdMap = (float*)Eigen::internal::aligned_malloc(
        this->numBlockWidth * this->numBlockHeight * sizeof(float));
    this->thresholdMapDuplication = (float*)Eigen::internal::aligned_malloc(
        this->numBlockWidth * this->numBlockHeight * sizeof(float));
}
...
int PointDetector::detect(const std::shared_ptr<Frame>& frame, int numPoints,
                          int32_t* const mask, IVisualizer* outputWrapper) {
    assert(mask != nullptr);
    ...
    if (this->worker) {
        ...
        for (int t = 0; t < numThreads; ++t) {
            ...
            this->worker->addJob(
                CalcThresholdParallel(*this, start, end, imgGradients[0]));
        }
        this->worker->wait();
    } else {
    ...
    for (int it = 0; it <= 1; ++it) {
        ...
        if (this->worker) {
            ...
            int numItems =
                (int)ceil((float)this->height[0] / (float)coarseWindow);
            ...
            for (int t = 0; t < numThreads; ++t) {
                ...
                this->worker->addJob(
                    Selector(*this, start, end, t, coarseWindow, mask));
            }
            this->worker->wait();
            ...
            numSelected =
                this->selectPixels(coarseWindow, mask, 0, this->height[0]);
        }
...
int PointDetector::selectPixels(const int winSize, int32_t* const mask,
...
int PointDetector::selectMaxPixel(const int row, const int col,
                                  const int winSize,
    ...
    this->selectMaxPixelRecursive(row, col, 0, 0, numFineWindows,
                                  fineWindowSize, this->numPyramids, maxLvl,
                                  salientPixels);
    ...
    if (lvl > 0) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            for (int j = 0; j < 2; ++j) {
                ...
                this->selectMaxPixelRecursive(
                    oy, ox, yIdx, xIdx, numFineWindows, fineWindowSize, newLvl,
...
int PointDetector::selectBestPixel(
    const std::vector<CandidatePixel>& salientPixels, const int row,
    const int col, const int numBlocks, const int lvl,
    int32_t* const mask) const {
    if (lvl > 0) {
        // try to select pixels in finer resolution
        int numSelected = 0;
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            for (int j = 0; j < 2; ++j) {
                ...
                numSelected += this->selectBestPixel(
                    salientPixels, y, x, finerNumBlocks, lvl - 1, mask);
            }
        }
...
void PointDetector::selectMaxPixelLvl(const int row, const int col,
                                      const int winSize, int& maxLvl,
                                      CandidatePixel& candidate) const {
    // window boundary
    const int min_x = std::min(std::max(col, 2), this->maxWidth[0] - 1);
    const int min_y = std::min(std::max(row, 2), this->maxHeight[0] - 1);
    const int max_x =
        std::min(std::max(col + winSize, 2), this->maxWidth[0] - 1);
    const int max_y =
        std::min(std::max(row + winSize, 2), this->maxHeight[0] - 1);
        ...
        for (int x = col; x < max_x; ++x) {
            ...
            for (int lvl = 0; lvl <= maxLvl; ++lvl) {
                ...
                if (lvl == 0) {
                    ...
                    if (!(v > 1 && v < this->maxHeight[lvl] && u > 1 &&
                          u < this->maxWidth[lvl]))
                        break;

                    // interpolate
                    mag =
                        bilinearInterpolation(gradMag, u, v, this->width[lvl]);
                }
...
void PointDetector::calcThresholds(const float* gradMag2, int yMin, int yMax) {
    for (int row = yMin; row < yMax; ++row) {
        ...
        for (int col = 0; col < this->numBlockWidth; ++col) {
            ...
            const int uEnd =
                std::min(uInit + this->blockSizeWidth, this->maxWidth[0]);
...
void PointDetector::smoothThresholdMap() {
    ...
    for (int row = 0; row < this->numBlockHeight; ++row) {
        ...
        for (int col = 0; col < this->numBlockWidth; ++col) {
            ...
            Kernel::convolveSafe<Kernel::Gaussian3x3>(
                this->thresholdMapDuplication, col, row, this->numBlockWidth,
...
PointDetector::CalcThresholdParallel::CalcThresholdParallel(
    PointDetector& thisDetector, int aBegin, int anEnd,
...
PointDetector::Selector::Selector(PointDetector& thisDetector, int aBegin,
                                  int anEnd, int threadNumber, int aWinSize,
                                  int32_t* const aMask)
    : detector(thisDetector),
      begin(aBegin),
      end(anEnd),
...
void PointDetector::Selector::operator()() {
    this->detector.numSelectedVector[this->threadNum] +=
        this->detector.selectPixels(this->winSize, this->mask, this->begin,
                                    this->end);
}
}  // namespace dsm
