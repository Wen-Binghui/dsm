...
class Processor {
   public:
    ...
    inline void run(EurocReader& reader, Undistorter& undistorter,
   ...
   private:
    inline void doRun(EurocReader& reader, Undistorter& undistorter,
                      QtVisualizer& visualizer, std::string& settingsFile) {
        int id = 0;
        cv::Mat image;
        double timestamp;
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 5) {
        ...
        std::cout
            << "The EurocExample requires at least 3 arguments: imageFolder, "
